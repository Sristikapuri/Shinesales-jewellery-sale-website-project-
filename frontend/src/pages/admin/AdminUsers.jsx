...
const AdminUsers = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#f3f6f4] to-white flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f3f6f4] to-white">
      {/* Header */}
      <section className="bg-gradient-to-r from-[#000000] to-[#000000] py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <div className="bg-[#b8860b] p-3 rounded-full">
                <Users className="h-8 w-8 text-[#f3f6f4]" />
              </div>
              <div>
                <h1 className="text-3xl font-bold text-white">Customer Management</h1>
                <p className="text-[#b8860b]">Manage customer accounts and information</p>
              </div>
            </div>
            
            <button className="bg-[#b8860b] text-[#0A400C] px-6 py-3 rounded-xl font-semibold hover:bg-white transition-colors flex items-center space-x-2">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-[#f3f6f4]">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#b8860b]">Customer</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#b8860b]">Email</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#b8860b]">Status</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#b8860b]">Joined</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-[#b8860b]">Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {filteredUsers.map((user) => (
                    <tr key={user.id} className="group hover:bg-[#b8860b]/50 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="bg-[#b8860b] p-2 rounded-full">
                            <Users className="h-5 w-5 text-[#f3f6f4]" />
                          </div>
                          <div>
                            {editingUser === user.id ? (
                            ...
                            {user.is_recent && (
                              <span className="inline-block bg-yellow-100 text-green-800 text-xs px-2 py-1 rounded-full mt-1">
                                New Customer
                              </span>
                            )}
                          </div>
